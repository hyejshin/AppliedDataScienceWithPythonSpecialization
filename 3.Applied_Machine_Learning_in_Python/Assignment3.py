...
def answer_four():
    ...
    confusion = confusion_matrix(y_test, y_pred_thres)
...
#     plt.show()
    ...
    truePositiveRate = np.interp(0.16, fpr_lr, tpr_lr)
...
#     plt.axes().set_aspect('equal')
#     plt.show()
    recallValue = np.interp(0.75, precision, recall)

    return (recallValue, truePositiveRate)
...
def answer_six():    
    ...
    grid_clf = GridSearchCV(LogisticRegression(), param_grid = grid_values, scoring='recall', cv=3)
