...
def doc_to_synsets(doc):
    ...
    for tag in pos:
        value = convert_tag(tag[1])
        syns = wn.synsets(tag[0], value)
        if syns:
            synList.append(syns[0])
    ...
    for synset1 in s1:
        ...
        for synset2 in s2:
            ...
            if score:
                scores.append(score)
        if scores:
            simSum.append(max(scores))
    
    return sum(simSum) / len(simSum)
...
def label_accuracy():
    ...
    return accuracy_score(label, paraphrases['Quality'])
...
def topic_names():
    ...
    titleList = ['Health', 'Science', 'Automobiles', 'Politics', 'Government', 'Travel', 
    ...
    for topic in ldamodel.top_topics(corpus)[:]:
        ...
        for title in titleList:
            tt = " ".join([itm[1] for itm in topic[0]])
            score = document_path_similarity(tt, title)
            if maxScore < score:
                maxScore = score
                suggestTitle = title
        suggestTitleList.append(suggestTitle)
